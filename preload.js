...
var Logger = {
     ...
     */
    log: (() =>
        console.log.bind(window.console, `%c+`, createLogPrefix("#81D4FA")))(),
    /**
     ...
     */
    info: (() =>
        console.info.bind(window.console, `%c+`, createLogPrefix("white")))(),
    /**
     ...
     */
    warn: (() =>
        console.warn.bind(window.console, `%c+`, createLogPrefix("#FDFD96")))(),
    /**
     ...
     */
    trace: (() =>
        console.trace.bind(window.console, `%c+`, createLogPrefix("orange")))(),
    /**
...
Logger.log(
    `Loaded Schoology Plus version ${chrome.runtime.getManifest().version}${
        getBrowser() != "Chrome" || chrome.runtime.getManifest().update_url
            ? ""
            : " (development version)"
    }`
);
        ...
        chrome.runtime.sendMessage(
            ...
            function (response) {
                if (response === undefined || response === null) {
                    ...
                    reject(
                        "Response is undefined or null. Last error: " +
                            chrome.runtime.lastError
                    );
                }
                ...
                let readBodyTask = new Promise(
                    (readBodyResolve, readBodyReject) => {
                        if (bodyReadError) {
    ...
    function onIntervalReset() {
        ...
        if (countToDequeue) {
            Logger.log(
                "Processing " +
                    countToDequeue +
                    " ratelimit-delayed queued requests"
            );
        }
...
var preload_schoologyPlusApiRateLimitedFetch = createFetchRateLimitWrapper(
    13,
    5000
);
...
async function fetchWithApiAuthentication(
    ...
    return await (useRateLimit
        ? preload_schoologyPlusApiRateLimitedFetch
        : backgroundPageFetch)(
        url,
        {
            headers: createApiAuthenticationHeaders(
                await getApiKeysInternal(),
                baseObj
            ),
        },
        bodyReadType
    ...
    return createElement(
        ...
        [
            createElement("input", ["form-submit", "splus-track-clicks"], {
                ...
                value: text,
                id: id,
                dataset: { splusTrackingLabel: "S+ Button" },
            }),
        ]
    );
}
...
async function getApiKeysInternal() {
    if (
        preload_globallyCachedApiKeys &&
        preload_globallyCachedApiKeys.length !== undefined
    ) {
        // API key object exists (truthy) and is an array (load completed)
        return preload_globallyCachedApiKeys;
    } else if (
        preload_globallyCachedApiKeys &&
        preload_globallyCachedApiKeys.then !== undefined
    ) {
...
function getUserId() {
    try {
        return Number.parseInt(
            new URLSearchParams(
                document
                    .querySelector("iframe[src*=session-tracker]")
                    .src.split("?")[1]
            ).get("id")
        );
    } catch (e) {
        Logger.warn(
            "Failed to get user ID from session tracker, using backup",
            e
        );
        try {
            return JSON.parse(
                document
                    .querySelector("script:not([type]):not([src])")
                    .textContent.split("=")[1]
            ).props.user.uid;
        } catch (e2) {
            Logger.error("Failed to get user ID from backup method", e2);
            throw new Error(
                "Failed to get user ID from backup method: " + e2.toString()
            );
        }
    }
}
...
async function getApiKeysDirect() {
    ...
    let html = await (
        await fetch(`https://${Setting.getValue("defaultDomain")}/api`, {
    ...
    let secret;
    if (
        (key = doc.getElementById("edit-current-key")) &&
        (secret = doc.getElementById("edit-current-secret"))
    ) {
        ...
        let submitData = new FormData(
            doc.getElementById("s-api-register-form")
        );
        let generateFetch = await fetch(
            `https://${Setting.getValue("defaultDomain")}/api`,
            {
                credentials: "same-origin",
                body: submitData,
                method: "post",
    ...
    chrome.storage.sync.get(null, (storageContents) => {
        ...
        modalContents = createElement("div", [], undefined, [
                ...
                new Setting(
                    ...
                    "select",
                    {
                        options: [
                            {
                                text:
                                    "Show Check Mark ✔ (Enables manual checklist)",
                                value: "check",
                            },
                            {
                                text:
                                    "Show Strikethrough (Doesn't allow manual checklist)",
                                value: "strikethrough",
                            },
                            {
            ...
            createElement("div", ["settings-buttons-wrapper"], undefined, [
                createButton("save-settings", "Save Settings", () =>
...
function Setting(
    ...
    this.name = name;
    this.getElement = () =>
        document.getElementById(`setting-input-${this.name}`);
    ...
    this.control = (() => {
        ...
        if (onload) {
            title.firstElementChild.value =
                onload(__storage[name]) || this.default;
        } else {
...
Setting.saveModified = function (
    ...
    chrome.storage.sync.set(newValues, () => {
        for (let settingName in newValues) {
            ...
            trackEvent(
                settingName,
                `set value: ${newValues[settingName]}`,
                "Setting"
            );
...
Setting.restoreDefaults = function () {
    if (
        ...
        for (let setting in __settings) {
            ...
            __settings[setting].onload(
                undefined,
                __settings[setting].getElement()
            );
        }
        location.reload();
    }
...
function createSvgLogo(...classes) {
    ...
    circle.setAttribute(
        "style",
        "fill: none; stroke-width: 35px; stroke: currentColor;"
    );
    ...
    line1.setAttribute(
        "style",
        "stroke-linecap: round; stroke-width: 35px; stroke: currentColor;"
    );
...
async function loadDependencies(name, dependencies) {
    ...
    while (!dependencies.every((d) => window.splusLoaded.has(d))) {
        Logger.debug(
            `Waiting to load ${name}: some of ${dependencies} not in ${window.splusLoaded}`
        );
